...
export default function HistoricalMatchExplorer({ archive, tournamentId, onBackToTournament }: Props) {
  ...
  return <div style={{minHeight:'100vh',background:'#07120f',color:'#eef8f3',fontFamily:'Inter,ui-sans-serif,system-ui,sans-serif'}}>
    <header style={{height:72,borderBottom:'1px solid rgba(255,255,255,.08)',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 28px',background:'#081410'}}>
      ...
      <button onClick={onBackToTournament} style={{border:'1px solid #264136',background:'#0b1713',color:'#9cefc0',borderRadius:8,padding:'8px 12px',cursor:'pointer'}}>← Tournament context</button>
    </header>
    <main style={{maxWidth:1180,margin:'0 auto',padding:28}}>
      <div style={{marginBottom:20}}><div style={{fontSize:9,letterSpacing:'.18em',color:'#7f968a',fontWeight:800}}>RECORDED FIXTURES</div><h1 style={{fontSize:30,margin:'6px 0'}}>World Cup {archive.tournaments.find(t=>t.year===Number(tournamentId.replace('wc-','')))?.year ?? ''} matches</h1><p style={{fontSize:11,color:'#8ea69b',lineHeight:1.6,margin:0}}>Archived competition records only. Scores and match context below are not simulation outputs.</p></div>
      ...
      <section style={{display:'grid',gap:8}}>{matches.map(m=>{const home=participation.get(m.homeTeamParticipationId),away=participation.get(m.awayTeamParticipationId);return <article key={m.id} style={{padding:15,border:'1px solid rgba(255,255,255,.07)',borderRadius:12,background:'#0a1713',display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:12,alignItems:'center'}}>
        <div><strong>{label(home)}</strong><div style={{fontSize:9,color:'#71877d',marginTop:5}}>{home?.confederation ?? 'Confederation unavailable'}</div></div>
        <div style={{textAlign:'center'}}><div style={{fontSize:18,fontWeight:900}}>{score(m)}</div><div style={{fontSize:9,color:'#9cefc0',marginTop:4}}>{stageLabels[m.stage]??m.stage}</div></div>
        <div style={{textAlign:'right'}}><strong>{label(away)}</strong><div style={{fontSize:9,color:'#71877d',marginTop:5}}>{away?.confederation ?? 'Confederation unavailable'}</div></div>
      ...
      {matches.length===0 && <div style={{padding:30,textAlign:'center',color:'#71877d',border:'1px solid rgba(255,255,255,.06)',borderRadius:12}}>No recorded fixtures match these filters.</div>}
