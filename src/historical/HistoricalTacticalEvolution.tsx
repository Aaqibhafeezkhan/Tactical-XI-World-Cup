...
export default function HistoricalTacticalEvolution({ onBack }: { onBack: () => void }) {
  ...
  return <div style={{minHeight:'100vh',background:'#07120f',color:'#eef8f3',fontFamily:'Inter,ui-sans-serif,system-ui,sans-serif'}}>
    <header style={{height:72,borderBottom:'1px solid rgba(255,255,255,.08)',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 28px',background:'#081410'}}>
      ...
      <button onClick={onBack} style={{border:'1px solid #264136',background:'#0b1713',color:'#9cefc0',borderRadius:8,padding:'8px 12px',cursor:'pointer'}}>← Historical explorer</button>
    </header>
    <main style={{maxWidth:1180,margin:'0 auto',padding:28}}>
      <div style={{marginBottom:20}}>
        ...
        <h1 style={{fontSize:30,margin:'6px 0'}}>How World Cup structures changed</h1>
      ...
      {error ? <section style={{padding:20,border:'1px solid rgba(245,184,88,.18)',borderRadius:14,background:'rgba(245,184,88,.05)',color:'#d6b978'}}><strong>Historical tactical evidence unavailable</strong><p style={{margin:'8px 0 0',lineHeight:1.6}}>{error}</p></section> : !data ? <div style={{padding:20,color:'#8ea69b'}}>Loading historical tactical records…</div> : <>
        ...
        <section style={{border:'1px solid rgba(255,255,255,.08)',borderRadius:16,background:'#0a1713',padding:18,marginBottom:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap',marginBottom:12}}>
            ...
            <select value={year} onChange={e=>{setYear(Number(e.target.value));setTeamQuery('')}} style={{background:'#07120f',border:'1px solid #264136',color:'#eef8f3',padding:'9px',borderRadius:8}}>{years.map(value=><option key={value}>{value}</option>)}</select>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))',gap:9,marginTop:12}}>
          ...
          {!teams.length && <div style={{padding:22,textAlign:'center',color:'#71877d',marginTop:10}}>No team formation records are available for this edition.</div>}
        </section>
